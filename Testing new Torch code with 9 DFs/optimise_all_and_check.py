...
def simulate_subject(v, X, j, cond1, cond2, a, b, sigma, k, vcor, ind_noise, model_type, reset_after, paradigm, N, ind, tuning_curves_indices, sub_num):
    ...
    for i in range(0, sub_num, batch_size):
        ...
        cov_voxels = (1 - vcor) * torch.eye(v) + vcor * torch.ones((v, v))
        mv_noise = torch.distributions.MultivariateNormal(loc=torch.zeros(v), covariance_matrix=cov_voxels)
        with torch.no_grad():
            mv_noise = torch.distributions.MultivariateNormal(loc=torch.zeros(v), covariance_matrix=cov_voxels)
            batch_noise = torch.stack([mv_noise.sample((T,)) for _ in range(current_batch_size)])
        
        # if vcor > 0:
        #     batch_noise = torch.stack([mv_noise.sample((noisy_pattern.shape[1],)) for i in range(current_batch_size)])
        # else: 
        #     batch_noise = torch.randn((current_batch_size, noisy_pattern.shape[1], v))
        
        base_noise = torch.randn((current_batch_size, T, v)) * ind_noise
        batch_noise = batch_noise * vcor
        noisy_pattern[i:i+current_batch_size] = out + base_noise + batch_noise
    if paradigm == 'face':
...
def produce_slopes_one_simulation(paradigm, model_type, sigma, a, b, k, vcor, ind_noise, n_jobs, n_simulations, v, tuning_curves_indices_near, sub_num, N, j, ind, reset_after, Kfold):
    ...
    N = 8
    y = simulate_subject(v, X, j, cond1, cond2, a, b, sigma, k, vcor, ind_noise, model_type, reset_after, paradigm, N, ind, tuning_curves_indices_near, sub_num)
    # print("end of produce_slopes_one_simulation")
    return produce_confidence_interval(y, 1, Kfold)

def produce_slopes_multiple_simulations(sigma, a, b, k, vcor, ind_noise, model_type, paradigm, n_jobs, n_simulations, v, tuning_curves_indices_all, sub_num, N, j, ind, reset_after, n_grad_sims, Kfold):
    ...
    # simulated = ...
    X = torch.pi
    results = torch.zeros((n_simulations, 9))
    for sim in range(n_simulations):
        ...
        if sim < n_grad_sims:
            result = produce_slopes_one_simulation(paradigm, model_type, sigma, a, b, k, vcor, ind_noise, n_jobs, n_simulations, v, tuning_curves_indices_near, sub_num, N, j, ind, reset_after, Kfold)
        else:
            with torch.no_grad():
                result = produce_slopes_one_simulation(
                    paradigm, model_type, sigma, a, b, k, vcor, ind_noise,
                    n_jobs, n_simulations, v,
                    tuning_curves_indices_near,
                    sub_num, N, j, ind, reset_after, Kfold
                )
        results[sim] = result
...
def optimise_model(
    ...
    optimiser = torch.optim.Adam([raw_a_param, raw_b_param, log_sigma_param, raw_k_param, raw_vcor_param, log_ind_noise_param], lr=lr)
    loss_list = []
    history = {
        'model_type': [],
        'loss': [],
    ...
    for step in range(n_steps):
        ...
        vcor_param = torch.sigmoid(raw_vcor_param)
        ind_noise_param = torch.exp(log_ind_noise_param)

        # print("before simulate")
        simulated_data = produce_slopes_multiple_simulations(sigma_param, a_param, b_param, k_param, vcor_param, ind_noise_param, model_type, paradigm, n_jobs, n_simulations, v, tuning_curves_indices_all, sub_num, N, j, ind, reset_after, n_grad_sims, Kfold)
        ...
        # print(f"  vcor: {vcor_param.item():.4f}, grad (raw): {raw_vcor_param.grad.item():.6f}")
        # print(f"  ind_noise: {ind_noise_param.item():.4f}, grad (raw): {log_ind_noise_param.grad.item():.6f}")
        ...
        history['vcor'].append(vcor_param.item())
        history['ind_noise'].append(ind_noise_param.item())
        if model_type in {2, 3, 5, 6, 8, 9, 11, 12}:
        ...
        # print(f"    vcor:  {vcor_param.item(): .4f}")
        # print(f"    ind_noise: {ind_noise_param.item(): .4f}")
...
raw_vcor_param = torch.nn.Parameter(torch.tensor(0.5).logit().unsqueeze(0))
log_ind_noise_param = torch.nn.Parameter(torch.tensor(-2.3026))
weights = 1/9 *torch.ones(9, requires_grad=True)
...
empirical_face_data = torch.tensor([-0.3657197926719654, -0.0337486931658741, -0.020352066375412297, -0.013396626790461809, 0.04041971183397424, 0.1837277393276265, 0.565, 0.62, 0.055], requires_grad=True)
empirical_grating_data = torch.tensor([-0.6499, -0.0525, -0.0610, 0.0085, -0.0233, 0.2581, 0.6, 0.747, 0.147])
...
torch.save({'tuning_indices': tuning_curves_indices_all
            }, 'noise_data.pt')
...
n_steps = 40
lr = 0.01
n_grad_sims = 10 #This is how many will be backpropagated for memory reasons

models_to_test = [4, 5, 6]
...
for i in models_to_test:
    ...
    history = optimise_model(
        ...
        raw_vcor_param=raw_vcor_param,
        log_ind_noise_param=log_ind_noise_param,
        n_steps=n_steps, 
        lr=lr, 
        model_type=i, 
        paradigm=paradigm, 
