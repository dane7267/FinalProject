...
def produce_basic_statistics(y, plag, Kfold):
    ...
    return AM, WC, BC, CP, AMS, AMA, CPPM, CPZP, ZPMINUSCP
...
#             for _ in range(10):
#                 print("1 loop of training for CPPM")
#                 optimizer.zero_grad()
#                 pred = model(X_train).squeeze()
#                 loss = nn.functional.binary_cross_entropy_with_logits(pred, y_train.squeeze())
...
def calculate_CPPM(y, Kfold):
    ...
    for sub in range(n_subs):
        ...
        F = torch.cat([cond1_p1, cond1_p2], dim=0)  # shape: (n//2, v)
        Y = torch.cat([
        ...
        Fm = F.mean(dim=1, keepdim=True)
        # print("Group 0 means:", Fm[Y == 0].squeeze())
        # print("Group 1 means:", Fm[Y == 1].squeeze())
...
def calculate_CPZP(y, Kfold):
    ...
    for sub in range(n_subs):
        ...
        for k in range(Kfold):
            ...
            # Closed-form solution: Ridge regression with bias term
            X_design = torch.cat([X_train, torch.ones(X_train.shape[0], 1, device=F.device)], dim=1)
            X_val_design = torch.cat([X_val, torch.ones(X_val.shape[0], 1, device=F.device)], dim=1)

            # Solve for weights w = (X^T X)^(-1) X^T y (optionally add regularization)
            w, *_ = torch.linalg.lstsq(X_design, y_train)  # shape: (v+1, 1)
...
def produce_confidence_interval(y, pflag, Kfold):
    AM, WC, BC, CP, AMS, AMA, CPPM, CPZP, ZPMINUSCP = produce_basic_statistics(y, pflag, Kfold)
    ...
    # print(torch.cat((slopes, torch.mean(CPPM).unsqueeze(0), torch.mean(CPZP).unsqueeze(0), torch.mean(ZPMINUSCP).unsqueeze(0))))
    return torch.cat((slopes, torch.mean(CPPM).unsqueeze(0), torch.mean(CPZP).unsqueeze(0), torch.mean(ZPMINUSCP).unsqueeze(0)))
